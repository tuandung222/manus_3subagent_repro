# Reading Guide

Use this guide to navigate documentation based on your objective.

## 1) Understand Scope and Success Criteria

- [`plans/REPRODUCTION_PLAN.md`](plans/REPRODUCTION_PLAN.md)
- [`plans/CODE_STRUCTURE_PLAN.md`](plans/CODE_STRUCTURE_PLAN.md)

## 2) Understand Architecture and Framework Choices

- [`architecture/AGENT_FRAMEWORK_ARCHITECTURE.md`](architecture/AGENT_FRAMEWORK_ARCHITECTURE.md)
- [`architecture/CODEACT_REACT_HYBRID_DESIGN.md`](architecture/CODEACT_REACT_HYBRID_DESIGN.md)
- [`architecture/MANUS_AGENT_ORCHESTRATION_DEEP_DIVE.md`](architecture/MANUS_AGENT_ORCHESTRATION_DEEP_DIVE.md)
- [`architecture/MANUS_PLANNER_PSEUDOCODE.md`](architecture/MANUS_PLANNER_PSEUDOCODE.md)

## 3) Understand Tracing and Dataset Construction

- [`plans/TRAINING_DATA_TRACING_PLAN.md`](plans/TRAINING_DATA_TRACING_PLAN.md)
- [`plans/TRAJECTORY_TRACING_TUNING_PLAN.md`](plans/TRAJECTORY_TRACING_TUNING_PLAN.md)

## 4) Review External Evidence Collection and Reproduction Gaps

- [`plans/MANUS_DOCUMENT_COLLECTION_AND_GAP_PLAN.md`](plans/MANUS_DOCUMENT_COLLECTION_AND_GAP_PLAN.md)

## 5) Follow the Notebook-Based Learning Path

- [`plans/JUPYTER_NOTEBOOK_EDUCATION_PLAN.md`](plans/JUPYTER_NOTEBOOK_EDUCATION_PLAN.md)
- [`../notebooks/education/README.md`](../notebooks/education/README.md)

## 6) Inspect Source Snapshots

- [`../references/README.md`](../references/README.md)
- [`../references/REFERENCE_SOURCES.csv`](../references/REFERENCE_SOURCES.csv)
